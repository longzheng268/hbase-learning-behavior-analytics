...
<!-- 【截图1】打开 IDEA → 展开左侧 src/main/java/com/learning 目录 → 截图展示 config/controller/dao/model/service/util 包结构 -->
...
<!-- 【截图2】打开终端 → 执行 hbase shell → 输入 describe 'learning:learning_behavior' → 截图展示 COLUMN FAMILIES DESCRIPTION 中的 detail 和 info 两个列族 -->
...
<!-- 【截图3】在 HBase Shell 中执行 scan 'learning:learning_behavior', {LIMIT => 5} → 截图展示 RowKey 格式如 S0001_9223372032560558807_LOGIN_0453 -->
...
<!-- 【截图4】在 IDEA 中打开 src/main/java/com/learning/dao/HBaseDao.java → 滚动到 scanByStudent 和 scanByStudentAndCourse 方法 → 截图展示 Scan + Filter 的代码 -->
...
<!-- 【截图5】浏览器打开 http://localhost:8080 → 左侧点击"数据概览" → 截图展示4个统计卡片（学生总数、课程总数、资源总数、行为记录数）和下方4个图表（每日趋势、时段分布、活跃排行、课程排行） -->

<!-- 【截图6】浏览器打开 http://localhost:8080 → 左侧点击"统计分析" → 截图展示登录次数柱状图、视频时长饼图、资源浏览横向柱状图、练习提交饼图、学习预警列表 -->

<!-- 【截图7】浏览器打开 http://localhost:8080 → 左侧点击"学生概况" → 下拉选择"杨春欣玥(S0021)" → 截图展示学生个人信息和行为类型饼图、学习趋势折线图 -->
...
<!-- 【截图8】打开终端 → 执行 hbase shell → 依次输入 list_namespace_tables 'learning' 和 describe 'learning:learning_behavior' → 截图展示两条命令的完整输出 -->
...
<!-- 【截图9】在 HBase Shell 中执行 count 'learning:learning_behavior' 和 scan 'learning:learning_behavior', {LIMIT => 3} → 截图展示数据条数和前3条记录的完整 RowKey、列族、值 -->
...
<!-- 【截图10】浏览器按 F12 打开开发者工具 → 切换到 Network 面板 → 在地址栏输入 http://localhost:8080/api/behavior/student/S0001 → 截图展示 Request URL 和 Response JSON 数据 -->
...
<!-- 【截图11】浏览器打开 http://localhost:8080 → 左侧点击"数据管理" → 截图展示：
  (a) Excel风格的行为数据表格（含筛选器、分页、编辑/删除按钮）
  (b) 下方的"新增行为记录"表单（含学生、课程、行为类型等下拉选择）
  (c) 学生信息管理表格（双击单元格可直接修改姓名、专业等） -->

<!-- 【截图12】浏览器打开 http://localhost:8080 → 左侧点击"多维查询" → 选择学生"S0001-张三"、课程"C001-大数据技术基础" → 点击查询 → 截图展示查询条件和下方结果表格 -->
